...
:: Kill any orphaned node processes running this project
for /f "tokens=2" %%a in ('wmic process where "commandline like '%%phantomx-trading%%' and name='node.exe'" get processid 2^>nul ^| findstr /r "[0-9]"') do (
    echo [*] Killing orphaned node process (PID: %%a)
    taskkill /F /PID %%a >nul 2>&1
)

:: Wait for ports and processes to fully release
