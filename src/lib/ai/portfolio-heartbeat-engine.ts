...
export class PortfolioHeartbeatEngine {
  ...
  private async tick(): Promise<void> {
    ...
    try {
      ...
      // 2. Phase 1: Fast scan — batch tickers + quick rank
      const phase1 = await this.phase1Scan(symbols);

      // 3. Build portfolio snapshot (positions + account)
      ...
      // 5. Phase 2: Detailed analysis on top 3
  ...
  private async executeAction(action: PortfolioTradeAction, portfolio: PortfolioSnapshot): Promise<void> {
    ...
    if (action.action === 'hold') {
      this.emit({
        ...
        tick: this.tickCount,
        phase: 'decide',
        data: { action: 'hold', symbol: action.symbol, reason: action.reason, confidence: action.confidence },
        timestamp: Date.now(),
      });
